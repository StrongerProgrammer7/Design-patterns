...
class Student_list_view < FXMainWindow
  ...
  private 
	...
	def initialize_filter(tab_frame)
		...
		@filter_git = @filters.add_filter_radioBtn(filter_area,"Наличие гита")
		@filter_mail = @filters.add_filter_radioBtn(filter_area,"Наличие почты")
		@filter_telegram = @filters.add_filter_radioBtn(filter_area,"Наличие телеграмма")
		@filter_phone = @filters.add_filter_radioBtn(filter_area,"Наличие телефона")
	...
	def initializeButtonTable_getData()
			@table_student.next_data_button.connect(SEL_COMMAND) do |sender,sel,data|
				if(self.num_page < self.max_page_data) then
    	 		...
    	 		if @filter_git[0].value == 2 then
    	 			showData(self.num_page,self.count_records)
    	 		else
    	 			showData_by_filter(@filter_git)
    	 		end
    	 		@selected_items=[]
    	 	end 
    	end
	...
	def initialize_control(tab_frame)
		...
		upd = button_control.createButton("Update")
		ed.disable
		del.disable
	...
	def event_filterStudent(update_btn)
