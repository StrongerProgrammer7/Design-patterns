...
class Table_auto < Table
	...
	def filter_data(filter_owner:nil,filter_model:nil,filter_mark:nil,filter_color:nil)
		fill_table(1,self.whole_entites_count,filter_owner:filter_owner,filter_model:filter_model,filter_mark:filter_mark,filter_color:filter_color)
	end
  ...
  def fill_table(num_page,count,filter_owner:nil,filter_model:nil,filter_mark:nil,filter_color:nil)
		clear_table((self.data[0].length-2),(self.data.length - 1))
		row = 0
		...
		loop do 
			if(self.data[ind]!=nil) then
				row = fill_table_rows(ind,row,filter_owner:filter_owner,filter_model:filter_model,filter_mark:filter_mark,filter_color:filter_color)
			end
   ...
   def fill_table_rows(ind,row,filter_owner:nil,filter_model:nil,filter_mark:nil,filter_color:nil)
   	if(filter_owner!=nil) then
			row = fill_table_by_filter(row,ind,filter_owner,1)
		elsif (filter_model != nil) then
			row = fill_table_by_filter(row,ind,filter_model,2)
		elsif (filter_mark != nil) then
			row = fill_table_by_filter(row,ind,filter_mark,3)
		elsif (filter_color != nil) then
			row = fill_table_by_filter(row,ind,filter_color,4)
		else
   ...
   def fill_table_by_filter(row,ind,filter,column)
   		if self.data[ind][column].include? filter then
   			fillRow(self.data[ind],row,5) 
				return row +=1
		end
		row
